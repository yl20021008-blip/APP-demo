    ...
    with get_connection() as conn:
        for row in df.to_dict(orient='records'):
            ...
            existing = conn.execute(
                select(words).where(
                    words.c.word == values['word'],
                    words.c.part_of_speech == values['part_of_speech'],
                    words.c.chapter == values['chapter'],
                )
            ).mappings().first()

            if existing is None:
                conn.execute(insert(words).values(**values))
                inserted_count += 1
            else:
                patch = {}
                if update_existing:
                    for key, value in values.items():
                        if key in {'word', 'part_of_speech', 'chapter', 'book_name', 'self_reported_known'}:
                            continue
                        old_value = existing.get(key)
                        if _is_blank(old_value) and not _is_blank(value):
                            patch[key] = value
                    if patch:
                        patch['enrichment_status'] = _infer_enrichment_status({**dict(existing), **patch})
