...
st.title('📥 上传词库')
st.caption('导入的是公共词库，所有学习者都能看到；但每个人的学习进度独立。')
...
col3.metric('已有音标数', int((preview_df['uk_phonetic'].fillna('').astype(str).str.strip().ne('') | preview_df['us_phonetic'].fillna('').astype(str).str.strip().ne('')).sum()))
...
if st.button('确认导入公共词库', type='primary'):
    try:
        result = import_words(preview_df, chapter=chapter, book_name=book_name, update_existing=update_existing, user_id=user_id)
    except Exception as exc:
        st.error(f'导入失败：{exc}')
    else:
