...
for uploaded_file in uploaded_files:
    try:
        ...
        phonetic_count = int(
            (
                df["uk_phonetic"].fillna("").astype(str).str.strip().ne("")
                | df["us_phonetic"].fillna("").astype(str).str.strip().ne("")
            ).sum()
        )
        parsed_items.append(
            {
                ...
                "df": df,
                "word_count": len(df),
                "chapter_count": df["chapter"].dropna().nunique() or 1,
...
summary_df = pd.DataFrame(
    [
        {
            "文件名": i["file_name"],
            "默认章节名": i["fallback_chapter"],
            "有效词数": i["word_count"],
            "表内章节数": i["chapter_count"],
            "已有音标数": i["phonetic_count"],
        }
...
col1.metric("成功读取文件", len(parsed_items))
col2.metric("读取失败文件", len(errors))
col3.metric("合计有效词条", int(summary_df["有效词数"].sum()) if not summary_df.empty else 0)
...
if parsed_items and st.button("确认批量导入公共词库", type="primary", use_container_width=True):
    ...
    for index, item in enumerate(parsed_items, start=1):
        ...
        status_text.write(f"正在导入文件 {index}/{len(parsed_items)}：{item['file_name']}")
        try:
            result = import_words(
                item["df"],
                chapter=item["fallback_chapter"],
                book_name=book_name,
            ...
            rows.append(
                {
                    "文件名": item["file_name"],
                    "新增": result["inserted"],
                    "补齐旧记录": result["updated"],
                    "重复": result["duplicated"],
                    "状态": "成功",
                }
            )
        except Exception as exc:
